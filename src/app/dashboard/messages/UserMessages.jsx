"use client"
import React, { useEffect, useState } from 'react'
import FilterUserMeSec from './FilterUserMeSec';
import { fetchAPIFunc } from '@/utils/clientRandomFunc';
import Link from 'next/link';
import { GoCheck } from "react-icons/go";
import CoverL from '@/app/componentes/global/smallComp/CoverL';
import ConfirmComp from '@/app/componentes/global/smallComp/ConfirmComp';
import { toast } from 'react-toastify';
import SendAdminMessage from '@/app/componentes/dashboard/SendAdminMessage';
import PaginationComp from '@/app/componentes/global/PaginationComp';
import { DomainPath } from '@/utils/DomainPath';

const UserMessages = () => {
	const [data, setData] = useState([])
	const [loading, setLoading] = useState(false)
	const [searchQ, setSearchQ] = useState({
		justNotReaded: true,
		page: 1,
		limit: 10,
		totalMessages: 0
	});
	const [checkControle, setCheckControle] = useState({
		messageID: null,
		showConfirm: false,
	});
	const [showSMForm, setShowSMForm] = useState({
		show: false,
		messageID: null,
		userID: null
	});

	const fetchData = async () => {
		try {
			setLoading(true);
			const messages = await fetchAPIFunc(`${DomainPath}/api/admin/messages?page=${searchQ.page}&limit=${searchQ.limit}&messageFor=users
				${!searchQ.justNotReaded ? "&checked=true" : ""}`, "GET", {}
			);
			const result = await messages.json();
			if (messages.status === 200) {
				setData(result.data)
				setSearchQ({...searchQ, totalMessages: parseInt(result.dataLength)})
			} else {
				toast.error("Failed To Get genres");
				setData(null)
			}
			setLoading(false);
		} catch (error) {
			setLoading(false);
			setData(null)
			console.log(error);
			return toast.error("something want wrong")
		}
	}

	const checkContent = (mID) => {
		setCheckControle({
			messageID: mID,
			showConfirm: true
		});
	}
	
	const handleConfirm = async () => {
		try {
			setLoading(true)
			setCheckControle({
				...checkControle,
				showConfirm: false
			})
			if (checkControle.messageID) {
				const respons = await fetchAPIFunc(`${DomainPath}/api/admin/messages`, "PUT", {id: checkControle.messageID});
				const result = await respons.json();
				if (respons.status === 200) {
					setLoading(false);
					toast.success(result.message);
					setCheckControle({
						messageID: null,
						showConfirm: false
					});
					await fetchData();
				} else {
					setLoading(false);
					setCheckControle({
						messageID: null,
						showConfirm: false
					});
					return toast.error(result.message);
				}
			}

		} catch (error) {
			console.log(error);
			setLoading(false);
			setCheckControle({
				messageID: null,
				showConfirm: false
			});
			return toast.error("something want wrong");
		}
		
	}
	const handleCansle = () => {
		setLoading(false);
		setCheckControle({
			messageID: null,
			showConfirm: false
		});
		return;
	}

	useEffect(() => {
		const run = async () => {
			await fetchData();
		}
		run();
	}, [searchQ.justNotReaded, searchQ.page, searchQ.limit]);

	return (
		<>
			{loading ? <CoverL/> : ""}
			{
				checkControle.showConfirm
				?
					<ConfirmComp data={{title: "Message Read State", body: "Are You Shur You Want To change This Message Read State", noBtn: "No", 
						yesBtn: "Update", bg: "d"}} callFuncs={{confirmFunc:handleConfirm, cansleFunc: handleCansle}}
					/>
				:
					''
			}
			{
				showSMForm.show
				?
				  <SendAdminMessage data={{info: showSMForm, closeForm: setShowSMForm}}/>
				:
					''
			}
			<div className='mt-4'>
				{
					data === null
					?
						<div className='p-3 b-g-d3 color-l rounded-1'>
							Failed To Get Data
						</div>
					:
						<>
							<FilterUserMeSec data={{searchQ, setSearchQ}}/>
							{
								data.length > 0 
								?
									<>
										<div className='table-container overflow-auto'>
											<table className='messages-table fs-main'>
												<thead>
													<tr className='color-g fw-semibold'>
														<td><span>Title</span></td>
														<td><span>Body</span></td>
														<td><span>UserID</span></td>
														<td><span>Reply To</span></td>
														<td><span>Action</span></td>
													</tr>
												</thead>
												<tbody className='table-body'>
													{
														data.map((message)=>(
															<tr key={message.m_id}>
																<td><span className='color-l '>{message.title}</span></td> 
																<td><span className='color-l text-start'>{message.body}</span></td>
																<td><span className='color-g text-capitalize fw-medium'>
																	<Link href={`/profile/${message.user_id}`} target='blank'>{message.user_id}</Link>
																</span></td>
																<td><span className={`${message.reply_to_id ? 'color-g' : 'color-l'}`}>
																	{message.reply_to_id ? message.reply_to_id : "None"}
																</span></td>
																<td>
																	<div className='d-flex align-items-center gap-3'>
																		<button className='color-g d-flex align-items-center bg-transparent border-0 p-0' onClick={() => setShowSMForm({
																			show: true,
																			messageID: message.m_id,
																			userID: message.user_id
																		})}>
																			Reply
																		</button>
																		<button className='color-dg d-flex align-items-center gap-1 bg-transparent border-0 p-0' onClick={
																			() => checkContent(message.m_id)
																		}>
																			{
																				message.m_checked ? 
																					<>
																						<span className='color-y'>Checked</span>
																						<GoCheck size={16} className='color-y'/>
																					</> 
																				: 
																					<span className='color-dg'>Check</span>
																				}
																		</button>
																	</div>
																</td>
															</tr>
														))
													}
												</tbody>
												<tfoot>
													<tr>
														<td className='w-100'>
															<PaginationComp data={{elePerPage: searchQ.limit, total: Number(searchQ.totalMessages), page: searchQ.page, 
																filterVals: searchQ, setFilterPage: setSearchQ}}
															/>
														</td>
													</tr>
												</tfoot>
											</table>
										</div>
									</>
								:
									<div className='p-3 b-g-d3 color-l rounded-1'>
										No Data Found
									</div>
							}
						</>
				}
				
			</div>
		</>
	)
}

export default UserMessages
